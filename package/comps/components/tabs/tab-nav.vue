  ...
  export default {
    ...
    render(h) {
      ...
      const scrollBtn = scrollable
        ? [
          <span class={['dm-tabs__nav-prev', scrollable.prev ? '' : 'is-disabled']} on-click={scrollPrev}><i class="dm__icon--left"></i></span>,
          <span class={['dm-tabs__nav-next', scrollable.next ? '' : 'is-disabled']} on-click={scrollNext}><i class="dm__icon--right"></i></span>
        ] : null;
      ...
      const tabs = this._l(panes, (pane, index) => {
        ...
        const btnClose = closable
          ? <span class="dm-icon-close" on-click={(ev) => { onTabRemove(pane, ev); }}></span>
          : null;
      ...
      return (
        <div class={['dm-tabs__nav-wrap', scrollable ? 'is-scrollable' : '', `is-${ this.rootTabs.tabPosition }`]}>
          {scrollBtn}
          <div class={['dm-tabs__nav-scroll']} ref="navScroll">
            <div
              class={['dm-tabs__nav', `is-${ this.rootTabs.tabPosition }`, stretch && ['top', 'bottom'].indexOf(this.rootTabs.tabPosition) !== -1 ? 'is-stretch' : '']}
              ref="nav"
              style={navStyle}
              role="tablist"
