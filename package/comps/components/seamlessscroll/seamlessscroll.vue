<template>
    <div class="dm__seamlessscroll" :style="{height: height + 'px'}">
        <div class="dm__seamlessscroll--box" :style="styles" :class="'dm__seamlessscroll--align-' + align">
            <div class="dm__seamlessscroll--item" v-html="lastItem" v-if="this.totalNum > 1"></div>
            <slot></slot>
            <div class="dm__seamlessscroll--item" v-html="firtstItem"></div>
        </div>
    </div>
</template>
...
export default {
  ...
  methods: {
    init () {
      this.destroy()
      this.items = this.$children.filter(item => item.$options.name === 'dm-seamlessscroll-item')
